...
int main(int argc, char *argv[]) {
  ...
  std::string ttyUSB0 = "/dev/ttyUSB0";
  ...
  ///ignore array
  str_optvalue = "";
  node->declare_parameter("ignore_array", false);
  ...
  node->declare_parameter("lidar_type", rclcpp::ParameterValue("YDLidar"));
  node->get_parameter("lidar_type", optval);
  laser.setlidaropt(LidarPropLidarType, &optval, sizeof(int));
  /// device type
  optval = YDLIDAR_TYPE_SERIAL;
  rcl_interfaces::msg::ParameterDescriptor desc;
  node->declare_parameter("device_type", "lidar", desc, true);
  node->get_parameter("device_type", optval);
  laser.setlidaropt(LidarPropDeviceType, &optval, sizeof(int));
  /// sample rate
  optval = 9;
  node->declare_parameter("sample_rate", 10.0);
  node->get_parameter("sample_rate", optval);
  laser.setlidaropt(LidarPropSampleRate, &optval, sizeof(int));
  /// abnormal count
  optval = 4;
  node->declare_parameter("abnormal_check_count", 10);
  ...
  node->declare_parameter("angle_max", 180.0, rcl_interfaces::msg::ParameterDescriptor(), true);
  node->get_parameter("angle_max", f_optvalue);
  laser.setlidaropt(LidarPropMaxAngle, &f_optvalue, sizeof(float));
  f_optvalue = -180.0f;
  node->declare_parameter("angle_min", -180.0, rcl_interfaces::msg::ParameterDescriptor(), true);
  ...
  /// unit: Hz
  f_optvalue = 10.f;
  rcl_interfaces::msg::ParameterDescriptor freqdesc;
  node->declare_parameter("frequency", 10.0, freqdesc,  true);
